...
public class AutoEnchantMod implements ClientModInitializer {
    private static final int ENCHANT_BUTTON_INDEX = 0;   // 只選第一個
    private static final int MIN_LEVEL_EXCLUSIVE = 50;   // XP 必須 > 50
    private static final int MIN_LAPIS_TO_START   = 28;  // 青金石必須 > 27
    ...
    public void onInitializeClient() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (handler instanceof EnchantmentScreenHandler) {
                runEnchantTick(client, (EnchantmentScreenHandler) handler);
            } else if (handler instanceof GrindstoneScreenHandler) {
                runGrindTick(client, (GrindstoneScreenHandler) handler);
            }
        });
    }
    ...
    // === 工具方法（皆用 clickSlot/clickButton，伺服器可記錄） ===
    ...
    private void moveViaPickup(ScreenHandler handler, int from, int to) {
        click(handler, from, 0, SlotActionType.PICKUP);
        click(handler, to,   0, SlotActionType.PICKUP);
    }
    private void moveToFirstEmptyMain(ScreenHandler handler, int from) {
        int target = findFirstEmptyMainSlot(handler);
        if (target != -1) moveViaPickup(handler, from, target);
    }
    private int countItemInMain(ScreenHandler handler, Item item) {
        int c = 0;
        for (int i = 0; i < handler.slots.size(); i++) {
            Slot s = handler.getSlot(i);
            if (isMainInventorySlot(s) && s.hasStack() && s.getStack().isOf(item)) c += s.getStack().getCount();
        }
        return c;
    }
    private int findFirstInMain(ScreenHandler handler, Item item) {
        for (int i = 0; i < handler.slots.size(); i++) {
            Slot s = handler.getSlot(i);
            if (isMainInventorySlot(s) && s.hasStack() && s.getStack().isOf(item)) return i;
        }
        return -1;
    }
    private int findFirstEnchantableInMain(ScreenHandler handler) {
        for (int i = 0; i < handler.slots.size(); i++) {
            Slot s = handler.getSlot(i);
            if (!isMainInventorySlot(s) || !s.hasStack()) continue;
            ItemStack st = s.getStack();
            if (st.isEnchantable() && !st.hasEnchantments() && !st.isOf(Items.ENCHANTED_BOOK)) return i;
        }
        return -1;
    }
    private int findFirstEnchantedInMain(ScreenHandler handler) {
        for (int i = 0; i < handler.slots.size(); i++) {
            Slot s = handler.getSlot(i);
            if (!isMainInventorySlot(s) || !s.hasStack()) continue;
            if (s.getStack().hasEnchantments()) return i;
        }
        return -1;
    }
    private boolean isMainInventorySlot(Slot s) {
        return s.inventory instanceof PlayerInventory && s.getIndex() >= 9 && s.getIndex() < 36;
    }
    private int findFirstEmptyMainSlot(ScreenHandler handler) {
        for (int i = 0; i < handler.slots.size(); i++) {
            Slot s = handler.getSlot(i);
            if (isMainInventorySlot(s) && !s.hasStack()) return i;
        }
        return -1;
    }
}
